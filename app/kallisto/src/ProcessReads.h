...
class SequenceReader {
 public:
  ...
  virtual bool fetchSequences(char* buf, const int limit,
                              std::vector<std::pair<const char*, int>>& seqs,
                              std::vector<std::pair<const char*, int>>& names,
                              std::vector<std::pair<const char*, int>>& quals,
                              std::vector<uint32_t>& flags, std::vector<std::string>& umis,
                              int& readbatch_id, bool full = false, bool comments = false) = 0;
...
class FastqSequenceReader : public SequenceReader {
 public:
  ...
  bool fetchSequences(char* buf, const int limit, std::vector<std::pair<const char*, int>>& seqs,
                      std::vector<std::pair<const char*, int>>& names,
                      std::vector<std::pair<const char*, int>>& quals, std::vector<uint32_t>& flags,
                      std::vector<std::string>& umis, int& readbatch_id, bool full = false,
                      bool comments = false);
 ...
 public:
  MasterProcessor(KmerIndex& index, const ProgramOptions& opt, MinCollector& tc,
      ...
      : tc(tc),
        ...
        tlencount(0),
        biasCount(0),
        maxBiasCount((opt.bias) ? 1000000 : 0),
    ...
    if (opt.batch_mode) {  // Set up recording of lengths individually for each batch
      if (opt.long_read) {
        ...
        // std::cerr << "Is this assignment of batchFlens space causing abort?" << std::endl;
        // std::cerr.flush();
        batchFlens_lr.assign(opt.batch_ids.size(),
                             std::vector<uint32_t>(index.target_lens_.size(), 0));
        batchFlens_lr_c.assign(opt.batch_ids.size(),
                               std::vector<uint32_t>(index.target_lens_.size(), 0));
        // std::cerr << "Passed assignment of batchFlens" << std::endl; std::cerr.flush();
        batchUnmapped_list.assign(opt.batch_ids.size(),
                                  std::vector<double>(3000000 / opt.batch_ids.size(), 0));
      } else {
        tlencounts.assign(opt.batch_ids.size(), 0);
    ...
    if (opt.bus_mode || opt.batch_mode) {
      ...
      if (opt.batch_mode) {
        if (opt.technology
                .empty()) {  // no_technology : No UMIs, no barcodes, so just have batch=barcode
          ...
          if (opt.busOptions.bc[0].fileno !=
              -1)  // We want to push batch at front of extracted barcode
        ...
        } else {  // We don't care about recording batch in barcode, so we follow the default
  ...
  std::vector<std::mutex>
      transfer_locks;  // one mutex per thread; for locking each thread's access to index.ecmapinv
  ...
  void update(const std::vector<uint32_t>& c, const std::vector<Roaring>& newEcs,
              ...
              std::vector<int>& flens_lr, std::vector<int>& flens_lr_c, std::vector<int>& bias,
              const PseudoAlignmentBatch& pseudobatch, std::vector<BUSData>& bv,
              std::vector<std::pair<BUSData, Roaring>> newB, int* bc_len, int* umi_len, int id = -1,
              int local_id = -1);
};
...
class ReadProcessor {
 public:
  ReadProcessor(const KmerIndex& index, const ProgramOptions& opt, const MinCollector& tc,
                MasterProcessor& mp, int id = -1, int local_id = -1);
...
class BUSProcessor {
 public:
  BUSProcessor(/*const*/ KmerIndex& index, const ProgramOptions& opt, const MinCollector& tc,
               MasterProcessor& mp, int id = -1, int local_id = -1);
...
class AlnProcessor {
 public:
  AlnProcessor(const KmerIndex& index, const ProgramOptions& opt, MasterProcessor& mp,
               const EMAlgorithm& em, const Transcriptome& model, bool useEM, int id = -1);
