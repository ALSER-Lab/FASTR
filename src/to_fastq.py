...
def create_base_map(gray_N=0, gray_A=3, gray_G=66, gray_C=129, gray_T=192):
    ...
    base_table[gray_T:255] = (
        gray_T  # Never reaches 254 (reserved for indicator of sequence start)
    )
    return base_table
...
def reverse_base_map(gray_N=0, gray_A=3, gray_G=66, gray_C=129, gray_T=192):
    ...
    reverse_map[gray_T:255] = ord(
        "T"
    )  # Never reaches 254 (reserved for indicator of sequence start)
    return reverse_map
        ...
        if mode == 1:
            ...
            while cursor < chunk_size:
                if safe_mode_flag:
                    ...
                    candidate = (
                        header_start_rel  # Then validate by finding \xff after this
                    )
                    ...
                    if (
                        xff_pos + 1 < len(data)
                        and data[xff_pos + 1 : xff_pos + 2] == b"\n"
                    ):
                    ...
                    while True:
                        ...
                        if next_at_rel != -1:
                            ...
                            if following_at_rel != -1 and (
                                ...
                                if (
                                    candidate_end_rel
                                    > seq_start_rel + MAX_CHUNK_EXTENSION
                                ):
                                    break
                                continue
                            ...
                            seq_end_rel = min(
                                len(data), seq_start_rel + MAX_CHUNK_EXTENSION
                            )
                ...
                if (
                    len(metadata_blocks) > 1
                    and current_metadata_idx < len(metadata_blocks) - 1
                ):
                ...
                if current_inverse_table is not None:
                    ...
                    quality_string = quality_to_ascii(
                        quality_scores, phred_offset
                    ).decode("ascii")
            ...
            while cursor < chunk_size:
                ...
                bases_line = (
                    b"".join(bases_parts).decode("utf-8", errors="replace").strip()
                )
                ...
                quality_line = data[line4_start:line4_end].decode(
            ...
            if header_index is not None and headers_file_path:
                ...
                if batch_start_idx < len(header_index):
                    ...
                    try:
                        ...
                        header_lines = (
                            batch_data.decode("utf-8", errors="ignore")
                            .strip()
                            .split("\n")
                        )
            ...
            if len(marker_positions) == 0:
                return (chunk_id, output_buffer.getvalue(), 0)
            ...
            for i in range(len(marker_positions)):
                ...
                if not header:  # Fallback to generation @seq header
                    ...
                    logger.warning(
                        f"Using fallback header for seq {sequence_count}: {header}"
                    )
            ...
            while cursor < chunk_size:
                ...
                if (
                    ...
                    and current_metadata.structure_template
                ):
...
def reconstruct_fastq(
    ...
    gray_C: int = 129,
    gray_T: int = 192,
    phred_alphabet_max: int = None,
    phred_offset: int = 33,
    chunk_size_mb: int = 8,
    num_workers: int = 4,
    mode: int = 2,
    mode3_headers_file: str = None,
    verbose: bool = False,
):
    """
    ...
    if phred_alphabet_max is None:
        if phred_from_metadata is not None:
            ...
            logger.info(
                f"No PHRED alphabet found in metadata, using default: {phred_alphabet_max}"
            )
    else:
        logger.info(
            f"Using user-specified PHRED alphabet (overriding metadata): {phred_alphabet_max}"
        )
    ...
    if metadata_blocks:
        ...
        for i, mb in enumerate(metadata_blocks):
            logger.info(
                f"  Block {i+1}: {mb.sequencer_type}, equation: {mb.scaling_equation}"
            )
    ...
    def chunk_generator():
        """Generate chunks for workers to process separately"""
        chunk_id = 0
        start_seq_idx = 0
        ...
        with open(input_path, "rb") as f:
            while pos < buffer_len:
                ...
                search_size = chunk_end - search_start + 1000
                ...
                if mode == 3:
                    # Mode 3 splits on \xff markers
                    last_marker = search_region.rfind(
                        b"\xff", 0, chunk_end - search_start
                    )
                ...
                if abs_end > abs_start:
                    f.seek(abs_start)
                    chunk_bytes = f.read(abs_end - abs_start)

                    if mode == 3 or mode in (1, 2) and safe_mode_flag:
                        # Count \xff markers for Mode 3
                        counted_seqs = chunk_bytes.count(b"\xff")
                    elif mode in (1, 2):
                        counted_seqs = chunk_bytes.count(
                            b"\n@"
                        )  # Unsafe mode
                    ...
                    if verbose:
                        logger.info(
                            f"Yielding chunk {chunk_id}: {abs_start}-{abs_end} "
                            + f"({abs_end-abs_start:,} bytes, {counted_seqs} seqs)"
                        )

                    yield (chunk_id, abs_start, abs_end, start_seq_idx)

                    start_seq_idx += counted_seqs
                    chunk_id += 1
    ...
    chunks_list = list(chunk_generator())
    total_sequences = 0
    try:
