...
OLD_ILLUMINA_PATTERN = re.compile(
    r"@([^:]+):(\d+):(\d+):(\d+):(\d+)#([A-Za-z0-9]+)/(\d+)"
)
SRA_PATTERN = re.compile(r"@([A-Z]+)(\d+)\.(\d+)\s+(\d+)(?:\s+(.*))?")

# SRA HYBRIDS
SRA_PACBIO_HIFI_PATTERN = re.compile(
    r"@([A-Z]+\d+)\.(\d+)\s+([^/]+)/(\d+)/ccs(?:\s+(.*))?"
)
SRA_PACBIO_CLR_PATTERN = re.compile(
    r"@([A-Z]+\d+)\.(\d+)\s+([^/]+)/(\d+)/(\d+)_(\d+)(?:\s+(.*))?"
)
SRA_ILLUMINA_PATTERN = re.compile(
    r"@([A-Z]+\d+)\.(\d+)(?:/[12])?\s+([^:]+):([^:]+):([^:]+):(\d+):(\d+):(\d+):(\d+)(?:\s+(.*))?"
)
...
def parse_ont_header(header: str) -> Tuple[Dict, str, str]:
    """Parse ONT (Oxford Nanopore Technologies) sequencer headers"""
...
def parse_sra_header(header: str) -> Tuple[Dict, str, str]:
    """Parse headers retrieved from the SRA database"""
    ...
    if extra_fields:
        structure = (
            f"{prefix}{accession_num}.{{REPEATING_1}} {{REPEATING_2}} {{REPEATING_3}}"
        )
...
def analyze_headers_for_pattern(
    headers: List[str], sample_size: int = 100
    ...
    if (
        delimiter == " "
        and len(split_headers[0]) > 0
        and all("." in h[0] for h in split_headers[: min(5, len(split_headers))])
    ):
        secondary_delimiter = "."
        new_split_headers = []
    ...
    if secondary_delimiter:
        structure_template = (
            structure_parts[0]
            + secondary_delimiter
            + delimiter.join(structure_parts[1:])
        )
    else:
...
def reconstruct_header_from_adaptive(
    structure: str, unique_id: str, delimiter: str
) -> str:
    """
...
def adaptive_compress_header(
    header: str, structure: str, delimiter: str, common_metadata: Dict
) -> Tuple[Dict, str]:
    """
...
def reconstruct_header_from_structure(
    structure: str, unique_id: str, sequencer_type: str
) -> str:
    """
...
def format_metadata_header(
    common_metadata: Dict, structure: str, sequencer_type: str
) -> str:
...
def parse_pacbio_hifi_sra_header(header: str) -> Tuple[Dict, str, str]:
    """Parse Pacbio HIFI headers retrieved from the SRA database"""
    ...
    if extra_fields:
        structure = (
            f"{accession}.{{REPEATING_1}} {movie}/{{REPEATING_2}}/ccs {{REPEATING_3}}"
        )
...
def parse_pacbio_clr_sra_header(header: str) -> Tuple[Dict, str, str]:
    """Parse Pacbio CLR headers retrieved from the SRA database"""
...
def parse_ont_sra_header(header: str) -> Tuple[Dict, str, str]:
    """Parse ONT headers retrieved from the SRA database"""
    parts = header.strip().split()
    if len(parts) < 2:
        return {}, header, ""
...
def parse_illumina_sra_header(header: str) -> Tuple[Dict, str, str]:
    """Parse Illumina headers retrieved from the SRA database"""
    ...
    y_pos = match.group(9)
    extra_fields = (
        match.group(10) if len(match.groups()) >= 10 and match.group(10) else ""
    )
