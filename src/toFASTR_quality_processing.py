...
def get_scaling_equation(
    scaling_method: str, custom_formula=None, phred_alphabet_max=41
) -> str:
...
def apply_quality_to_bases(
    ...
    if scaling_method == "one_hot":
        scale_factors = np.zeros_like(quality_scores, dtype=np.uint8)

    # Calculate scale factors based on method
    if scaling_method == "custom":
        if custom_formula is None:
            raise Exception(
                "ERROR: custom scaling method requires --custom_formula argument"
            )

        # Parse and evaluate custom formula
        scale_factors = apply_clamping_to_equation(
            parse_custom_formula(custom_formula, quality_scores)
        )
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            LOG_DICT = np.clip(
                1 + 62 * (np.log(x + 1) / np.log(phred_alphabet_max + 1)), 1, 63
            ).astype(np.uint8)
...
def validate_and_adjust_formula(formula: str, phred_alphabet_max: int) -> str:
    ...
    try:
        ...
        if raw_min < 1 or raw_max > 63:
            logger.warning(
                f"Formula produces values outside 1-63 range before clipping!"
            )
            logger.warning(f"Raw output range: {raw_min:.2f} to {raw_max:.2f}")
            logger.debug(
                f"  Values clamped: {clamped_low + clamped_high}/{total_values} "
                + f"({100 * (clamped_low + clamped_high) / total_values:.1f}%)"
            )

            if clamped_low > 0:
                logger.info(
                    f"    - {clamped_low} values below 1 (will be clamped to 1)"
                )
            if clamped_high > 0:
                logger.info(
                    f"    - {clamped_high} values above 63 (will be clamped to 63)"
                )
